...
def train(hyp, tb_writer, opt, device):
    ...
    results = (
        0, 0, 0, 0, 0, 0, 0, 0
    )  # 'P', 'R', 'mAP50', 'mAP', 'mDepth', 'val GIoU', 'val Objectness', 'val Classification'
    ...
    for epoch in range(
        ...
        if rank in [-1, 0]:
            print(('\n' + '%10s' * 9) %
                  ('Epoch', 'gpu_mem', 'GIoU', 'obj', 'cls', 'depth', 'total',
                   'targets', 'img_size'))
        ...
        for i, (
            ...
            imgs[:,
                 3, :, :] /= 255.0  # Rescale depth channel. Max depth found in inputs was 202 m
            ...
            if opt.multi_scale:
                ...
                if sf != 1:
                    ...
                    tmp_imgs = torch.empty(
                        (N, C, round(H * sf), round(W * sf)),
                        dtype=imgs.dtype,
                        layout=imgs.layout,
                        device=imgs.device)
                    tmp_imgs[:, :, :, :] = 0
                    tmp_imgs[:, 3:, :, :] = resize_dm_torch(
                        imgs[:, 3:, :, :], sf)
                    tmp_imgs[:, :3, :, :] = F.interpolate(imgs[:, :3, :, :],
                                                          size=unpad_shape,
                                                          mode='bilinear',
                                                          align_corners=False)
                    ...
                    targets = multiscale_targets(targets, unpad_shape,
                                                 (dw, dh), ns)
            # Forward
            ...
            s = ('%10s' * 2 + '%10.4g' * 7) % ('%g/%g' % (epoch, epochs - 1),
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                results, maps, times, inf_imgs = test.test(
                    opt.data,
                ...
                with open(results_file, 'a') as f:
                    f.write(
                        s + '%10.4g' * 9 % results + '\n'
                    )  # P, R, mAP, F1, mDepth, test_losses=(GIoU, obj, cls, depth)
                ...
                if tb_writer:
                    tags = [
                        ...
                        'train/depth_loss', 'metrics/precision',
                        'metrics/recall', 'metrics/mAP_0.5',
                        'metrics/mAP_0.5:0.95', 'metrics/mean_depth',
                    ...
                    if not isinstance(inf_imgs[0], type(None)):
                        tb_writer.add_image('Prediction',
                                            inf_imgs[1],
                                            dataformats='HWC',
                                            global_step=epoch)
                        tb_writer.add_image('Ground-Truth',
                                            inf_imgs[0],
                                            dataformats='HWC',
                                            global_step=epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg',
                        type=str,
                        default='models/yolov4m-rgbd.yaml',
                        help='model.yaml path')
    parser.add_argument('--data',
                        type=str,
                        default='data/amz_data_splits.yaml',
                        help='data.yaml path indicating the data splits')
