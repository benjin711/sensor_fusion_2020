...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    wh = torch.tensor(
        np.concatenate([
            l[:, 4:6] * s for s, l in zip(shapes * scale, dataset.labels)
        ])).float()  # wh
...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    tcls, tdepth, tbox, indices, anchors = build_targets(p, targets,
                                                         model)  # targets
...
def non_max_suppression(prediction,
    ...
    for xi, (x, path,
        ...
        if multi_label:
            ...
            x = torch.cat(
                (box[i], x[i, j + 6, None], j[:, None].float(), x[i, 4, None]),
                1)
        else:  # best class only
            conf, j = x[:, 6:].max(1, keepdim=True)
            x = torch.cat((box, conf, j.float(), x[:, 4, None]),
        ...
        if merge and (1 < n <
            ...
            try:  # update boxes as boxes(i,4) = weights(i,n) * boxes(n,4)
                ...
                # When averaging over the boxes also average over the depth (weighted mean depth)
                x[i, 6] = (torch.mm(weights, x[:, 6, None]).float() /
                           weights.sum(1, keepdim=True)).t()
...
def create_pretrained(f='weights/best.pt',
                      s='weights/pretrained.pt'
...
def kmean_anchors(path='./data/coco128.yaml',
    ...
    wh0 = np.concatenate(
        [l[:, 4:6] * s for s, l in zip(shapes, dataset.labels)])  # wh
...
def plot_one_box(x,
    ...
    if img.shape[2] > 3:
        img = copy(img[:, :, :3]).astype(np.uint8)
    ...
    if depth:
        ...
        # cv2.rectangle(img, c1, c2, color, -1, cv2.LINE_AA)  # filled
        cv2.putText(img,
                    f"{depth:.1f}", (c1[0], c2[1] + font_size[1]),
                    0,
                    sf, [255, 255, 255],
                    thickness=tf,
                    lineType=cv2.LINE_AA)
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            conf = None if gt else image_targets[:,
                                                 7]  # check for confidence presence (gt vs pred)
