...
def attempt_load(weights, map_location=None):
    ...
    for w in weights if isinstance(weights, list) else [weights]:
        ...
        temp = torch.load(w, map_location=map_location)['model'].float().fuse()
        model_info = temp.info()
        model.append(temp.eval())  # load FP32 model

    if len(model) == 1:
        return model[-1], model_info  # return model
    else:
        print('Ensemble created with %s\n' % weights)
        for k in ['names', 'stride']:
            setattr(model, k, getattr(model[-1], k))
        return model, model_info  # return ensemble
